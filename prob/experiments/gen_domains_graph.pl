...
sub make_report {
  ...
  foreach my $domain (@$domains) {
    ...
    $col_precision = $data->{'header'}->{'cols'}->{'precision'};
    ...
    my $data_stats = gen::summary_data_of_data($data, [
						       '_epoch total (real s)',
						       $pol_cols->[$policy_num],
						      ]);
    ...
    $col_stats_precision = $data_stats->{'header'}->{'cols'}->{'precision'};
  ...
  my $cname = "_epoch total (real s)";
  my $col_stats_realtime_qlower = $data_stats_all->{'header'}->{'cols'}->{"_q_lower $cname"};
  my $col_stats_realtime_qupper = $data_stats_all->{'header'}->{'cols'}->{"_q_upper $cname"};
  my $col_stats_realtime_median = $data_stats_all->{'header'}->{'cols'}->{"_median $cname"};
  my $col_stats_realtime_min = $data_stats_all->{'header'}->{'cols'}->{"_min $cname"};
  my $col_stats_realtime_max = $data_stats_all->{'header'}->{'cols'}->{"_max $cname"};
  ...
  $data_stats_all->foreach_line
    (
     sub {
       ...
       my $amin = $line->get_data_at_index($col_stats_realtime_qlower);
       my $amax = $line->get_data_at_index($col_stats_realtime_qupper);
  ...
  my $file_template = $config::DIR_TEMPLATES . "/plot_domains_template.gnu";
  ...
  my $gnu_stats = util::fill_template($template,
				      {
				       ...
				       'COL_STATS_PRECISION' => $col_stats_precision + 1,
				       'COL_STATS_REALTIME' => $col_stats_realtime + 1,
				       'COL_STATS_BELIEF' => $col_stats_belief + 1,
				       'COL_STATS_RELATIVE' => $col_stats_relative + 1,
				       'COL_STATS_REALTIME_MIN' => $col_stats_realtime_min + 1,
				       'COL_STATS_REALTIME_MAX' => $col_stats_realtime_max + 1,
				       'COL_STATS_REALTIME_MEDIAN' => $col_stats_realtime_median + 1,
				       'COL_STATS_REALTIME_QLOWER' => $col_stats_realtime_qlower + 1,
				       'COL_STATS_REALTIME_QUPPER' => $col_stats_realtime_qupper + 1,
				      });
  ...
  my $gnu_stats_eps = util::fill_template($gnu_stats,
					  {
					   ...
					   #'TERM'      => 'png crop'
					   'GRAPH_OUT'  => "$dir/stats_graph_all.eps",
